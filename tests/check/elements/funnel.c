...
static gboolean
eos_event_func (GstPad * pad, GstEvent * event)
{
...
GST_START_TEST (test_funnel_eos)
{
  ...
  fail_unless (gst_pad_push (td.mysrc1, gst_buffer_new ()) ==
      GST_FLOW_UNEXPECTED);
  fail_unless (gst_pad_push (td.mysrc2, gst_buffer_new ()) == GST_FLOW_OK);
  ...
  fail_unless (gst_pad_push_event (td.mysrc1, gst_event_new_flush_stop ()));
