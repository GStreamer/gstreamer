        ...
        let message_payload = match message_type {
            PtpMessageType::ANNOUNCE => {
                if data.len() < 30 {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidInput,
                        format!("Too short PTP ANNOUNCE message {} < 30", data.len()),
                    ));
                }
            ...
            PtpMessageType::SYNC => {
                if data.len() < 10 {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidInput,
                        format!("Too short PTP SYNC message {} < 10", data.len()),
                    ));
                }
            ...
            PtpMessageType::FOLLOW_UP => {
                if data.len() < 10 {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidInput,
                        format!("Too short PTP FOLLOW_UP message {} < 10", data.len()),
                    ));
                }
            ...
            PtpMessageType::DELAY_REQ => {
                if data.len() < 10 {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidInput,
                        format!("Too short PTP DELAY_REQ message {} < 10", data.len()),
                    ));
                }
