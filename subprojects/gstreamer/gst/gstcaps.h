 ...
 * ### Extra features:
 *
 * #### Gst.Caps constructor
 *
 * ``` python
 * Gst.Caps(string: str) -> Gst.Caps
 * Gst.Caps(caps: Gst.Caps) -> Gst.Caps
 * Gst.Caps(structure: Gst.Structure) -> Gst.Caps
 * Gst.Caps(structures: Listp[Gst.Structure]) -> Gst.Caps
 * ```
 *
 * The Gst.Caps constructor has been overridden to provide a more pythonic
 * interface:
 *
 * ``` python
 * # Create from string
 * caps = Gst.Caps("video/x-raw, width=640, height=480")
 *
 * # Copy existing caps
 * caps2 = Gst.Caps(caps)
 *
 * # Create from structure
 * struct = Gst.Structure("video/x-raw", width=640, height=480)
 * caps = Gst.Caps(struct)
 *
 * # Create from list of structures
 * caps = Gst.Caps([struct1, struct2])
 * ```
 *
 * #### Gst.Caps.__getitem__
 *
 * ``` python
 * def __getitem__(self, index: int) -> Gst.Structure:
 * ```
 *
 * Access structure by index.
 *
 * #### Gst.Caps.__len__
 *
 * ``` python
 ...
 * ```
 *
 * Get number of structures in caps:
 *
 * ``` python
 * caps = Gst.Caps("video/x-raw, width=640; video/x-h264")
 *
 * # Get number of structures
 * num_structures = len(caps)
 *
 * # Access structure by index
 * first_struct = caps[0]
 *
 ...
 *
 * ``` python
 * def get_structure(self, index: int) -> Gst.Structure:
 * ```
 *
 * Returns a structure that maintains a reference to its parent caps.
 *
 * **Since**: 1.28
 *
 * #### Gst.Caps.writable_structure
 *
 * ``` python
 * def writable_structure(self, index: int) -> Gst.Structure:
 * ```
 *
 * Returns a structure that can be modified and must be used within a context manager.
 *
 * **Since**: 1.28
 *
 * ``` python
 * caps = Gst.Caps("video/x-raw, width=640")
 *
 * # Read-only access
 * struct = caps.get_structure(0)
 *
 * # Writable access (must use context manager)
 * with caps.writable_structure(0) as struct:
 *     struct["height"] = 480
 * assert caps[0]["height"] == 480
 * ```
 *
 * {{ END_LANG.md }}
 */
