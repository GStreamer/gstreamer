...
static gboolean gst_d3d12_interlace_propose_allocation (GstBaseTransform *
    trans, GstQuery * decide_query, GstQuery * query);
static gboolean gst_d3d12_interlace_decide_allocation (GstBaseTransform *
...
gst_d3d12_interlace_class_init (GstD3D12InterlaceClass * klass)
{
  ...
  trans_class->decide_allocation =
      GST_DEBUG_FUNCPTR (gst_d3d12_interlace_decide_allocation);
...
static gboolean
gst_d3d12_interlace_propose_allocation (GstBaseTransform * trans,
...
static gboolean
gst_d3d12_interlace_decide_allocation (GstBaseTransform * trans,
    GstQuery * query)
{
  auto filter = GST_D3D12_BASE_FILTER (trans);
...
gst_d3d12_interlace_prepare_convert (GstD3D12Interlace * self,
    GstCaps * in_caps, const GstVideoInfo * in_info, GstVideoInfo * weave_info)
{
  ...
  auto ctx = std::make_shared < InterlaceConvCtx > (filter->device);
    ...
    GstVideoInfo * out_info)
{
...

  {
    ...
    if (!gst_d3d12_interlace_prepare_convert (self, incaps, &priv->in_info,
            &priv->weave_info)) {
      return FALSE;
    }
    ...
    priv->weave = gst_d3d12_weave_interlace_new (filter->device,
