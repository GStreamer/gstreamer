...
/**
 * SECTION:element-ssdtensordec
 * @short_description: Decode tensors from inference using a SSD neural network
 *
...
GST_DEBUG_CATEGORY_STATIC (ssd_tensor_dec_debug);
#define GST_CAT_DEFAULT ssd_tensor_dec_debug
GST_ELEMENT_REGISTER_DEFINE (ssd_tensor_dec, "ssdtensordec",
    GST_RANK_SECONDARY, GST_TYPE_SSD_TENSOR_DEC);

GST_DEBUG_CATEGORY_STATIC (ssd_object_detector_debug);
GST_ELEMENT_REGISTER_DEFINE (ssd_object_detector, "ssdobjectdetector",
    GST_RANK_NONE, GST_TYPE_SSD_OBJECT_DETECTOR);
/* GstSsdTensorDec properties */
enum
{
  PROP_0,
  PROP_LABEL_FILE,
...
#define GST_SSD_TENSOR_DEC_DEFAULT_SCORE_THRESHOLD       0.3f   /* 0 to 1 */
#define GST_SSD_TENSOR_DEC_DEFAULT_SIZE_THRESHOLD       0.9f    /* 0 to 1 */

static GstStaticPadTemplate gst_ssd_tensor_dec_src_template =
...
static GstStaticPadTemplate gst_ssd_tensor_dec_sink_template =
            ...
            "(GstCaps)["
             ...
             "(GstCaps)["
              "tensor/strided,"
                ...
                "dims=(int)<[0, max], [0, max]>,"
                "dims-order=(string)row-major,"
                "type=(string)float32;],"
            "(GstCaps)["
              "tensor/strided,"
                "tensor-id="GST_MODEL_OBJECT_DETECTOR_BOXES","
                "dims=(int)<[0, max], [0, max], 4>,"
                "dims-order=(string)row-major,"
                "type=(string)float32;],"
            "(GstCaps)["
              "tensor/strided,"
...
static void gst_ssd_tensor_dec_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_ssd_tensor_dec_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec);
static void gst_ssd_tensor_dec_finalize (GObject * object);
static GstFlowReturn gst_ssd_tensor_dec_transform_ip (GstBaseTransform *
    trans, GstBuffer * buf);
static gboolean gst_ssd_tensor_dec_process (GstBaseTransform * trans,
    GstBuffer * buf);
static gboolean
gst_ssd_tensor_dec_set_caps (GstBaseTransform * trans, GstCaps * incaps,
    GstCaps * outcaps);

G_DEFINE_TYPE (GstSsdTensorDec, gst_ssd_tensor_dec, GST_TYPE_BASE_TRANSFORM);

static void
gst_ssd_tensor_dec_class_init (GstSsdTensorDecClass * klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstElementClass *element_class = (GstElementClass *) klass;
  GstBaseTransformClass *basetransform_class = (GstBaseTransformClass *) klass;

  GST_DEBUG_CATEGORY_INIT (ssd_tensor_dec_debug, "ssdtensordec",
      0, "ssdtensordec");
  gobject_class->set_property = gst_ssd_tensor_dec_set_property;
  gobject_class->get_property = gst_ssd_tensor_dec_get_property;
  gobject_class->finalize = gst_ssd_tensor_dec_finalize;

  /**
   * GstSsdTensorDec:label-file
   *
   * Label file
   *
   * Since: 1.24
  ...
  /**
   * GstSsdTensorDec:score-threshold
   *
  ...
  g_object_class_install_property (G_OBJECT_CLASS (klass), PROP_SCORE_THRESHOLD,
      g_param_spec_float ("score-threshold",
          "Score threshold",
          "Threshold for deciding when to remove boxes based on score",
          0.0, 1.0, GST_SSD_TENSOR_DEC_DEFAULT_SCORE_THRESHOLD, (GParamFlags)
          (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

  /**
   * GstSsdTensorDec:size-threshold
   *
  ...
  g_object_class_install_property (G_OBJECT_CLASS (klass), PROP_SIZE_THRESHOLD,
      g_param_spec_float ("size-threshold",
          "Size threshold",
          "Threshold for deciding when to remove boxes based on proportion of the image",
          0.0, 1.0, GST_SSD_TENSOR_DEC_DEFAULT_SIZE_THRESHOLD, (GParamFlags)
          (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
  ...
  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&gst_ssd_tensor_dec_sink_template));

  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&gst_ssd_tensor_dec_src_template));
  basetransform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_ssd_tensor_dec_transform_ip);
  basetransform_class->set_caps =
      GST_DEBUG_FUNCPTR (gst_ssd_tensor_dec_set_caps);
}

static void
gst_ssd_tensor_dec_init (GstSsdTensorDec * self)
{
  self->size_threshold = GST_SSD_TENSOR_DEC_DEFAULT_SIZE_THRESHOLD;
  self->score_threshold = GST_SSD_TENSOR_DEC_DEFAULT_SCORE_THRESHOLD;
  GST_PAD_UNSET_ACCEPT_INTERSECT (self->basetransform.sinkpad);
}

static void
gst_ssd_tensor_dec_finalize (GObject * object)
{
  GstSsdTensorDec *self = GST_SSD_TENSOR_DEC (object);

  g_free (self->label_file);
  g_clear_pointer (&self->labels, g_array_unref);

  G_OBJECT_CLASS (gst_ssd_tensor_dec_parent_class)->finalize (object);
}
...
static void
gst_ssd_tensor_dec_set_property (GObject * object, guint prop_id,
...
static void
gst_ssd_tensor_dec_get_property (GObject * object, guint prop_id,
...
static gboolean
gst_ssd_tensor_dec_get_tensors (GstSsdTensorDec * object_detector,
...
gst_ssd_tensor_dec_set_caps (GstBaseTransform * trans, GstCaps * incaps,
    GstCaps * outcaps)
{
  GstSsdTensorDec *self = GST_SSD_TENSOR_DEC (trans);
...
gst_ssd_tensor_dec_transform_ip (GstBaseTransform * trans, GstBuffer * buf)
{
  if (!gst_base_transform_is_passthrough (trans)) {
    if (!gst_ssd_tensor_dec_process (trans, buf)) {
...
static void
extract_bounding_boxes (GstSsdTensorDec * self, gsize w, gsize h,
...
static gboolean
gst_ssd_tensor_dec_process (GstBaseTransform * trans, GstBuffer * buf)
{
  GstSsdTensorDec *self = GST_SSD_TENSOR_DEC (trans);
  ...
  if (!gst_ssd_tensor_dec_get_tensors (self, buf,
...
gst_ssd_object_detector_class_init (GstSsdObjectDetectorClass * klass)
{
  GST_DEBUG_CATEGORY_INIT (ssd_object_detector_debug, "ssdobjectdetector",
      0, "ssd object detector category");

  gst_element_class_set_static_metadata (GST_ELEMENT_CLASS (klass),
      "SSD MobileNet Object Detector tensor decoder (Deprecated)",
      "Tensordecoder/Video",
      "Apply tensor output from inference to detect objects in video frames",
      "Aaron Boxer <aaron.boxer@collabora.com>, Marcus Edel <marcus.edel@collabora.com>");

  gst_type_mark_as_plugin_api (GST_TYPE_SSD_TENSOR_DEC, 0);
}

static void
gst_ssd_object_detector_init (GstSsdObjectDetector * self)
{
  GST_CAT_WARNING (ssd_object_detector_debug, "ssdobjectdetector is "
      "deprecated, use ssdtensordec instead");
}
