      ...
      for (l = profiles.head; l; l = l->next)
        {
          ...
          if (v4l2object->ioctl(v4l2object->video_fd, VIDIOC_S_CTRL, &control) <
              0)
            {
      ...
      while ((oldest_frame = gst_video_encoder_get_oldest_frame(encoder)) &&
                                               ...
                                               system_frame_number))
        {
          ...
          if (!warned)
            {
              g_warning
                ("%s: Too old frames, bug in encoder -- please file a bug",
      ...
      if (self->output_flow != GST_FLOW_OK &&
          ...
          self->output_flow != GST_V4L2_FLOW_LAST_BUFFER)
        {
          GST_DEBUG_OBJECT(self,
                           "Processing loop stopped with error: %s, leaving",
                           gst_flow_get_name(self->output_flow));
          ret = self->output_flow;
          goto drop;
        }
    ...
    if (!gst_buffer_pool_is_active(opool))
      {
        ...
        gst_buffer_pool_config_set_params(config, self->input_state->caps,
                                          self->v4l2output->info.vinfo.size,
                                          min, min);
  ...
  if (task_state == GST_TASK_STOPPED || task_state == GST_TASK_PAUSED)
    {
      ...
      if (!gst_pad_start_task(encoder->srcpad,
                              (GstTaskFunction) gst_v4l2_video_enc_loop,
                              self,
  ...
  if (frame->input_buffer)
    {
      ...
      {
        GstBufferPool *opool =
          gst_v4l2_object_get_buffer_pool(self->v4l2output);
        ret =
          gst_v4l2_buffer_pool_process(GST_V4L2_BUFFER_POOL(opool),
                                       &frame->input_buffer,
                                       &frame->system_frame_number);
        gst_object_unref(opool);
      }
...
static gboolean
gst_v4l2_video_enc_propose_allocation(GstVideoEncoder *encoder, GstQuery *query)
{
  GstV4l2VideoEnc *self = GST_V4L2_VIDEO_ENC(encoder);
  gboolean ret = FALSE;
...
gst_v4l2_video_enc_subinstance_init(GTypeInstance *instance, gpointer g_class)
{
  ...
  self->v4l2output = gst_v4l2_object_new(GST_ELEMENT(self),
                                         GST_OBJECT
                                         (GST_VIDEO_ENCODER_SINK_PAD(self)),
                                         V4L2_BUF_TYPE_VIDEO_OUTPUT,
                                         klass->default_device,
                                         gst_v4l2_get_output,
  ...
  self->v4l2capture = gst_v4l2_object_new(GST_ELEMENT(self),
                                          GST_OBJECT
                                          (GST_VIDEO_ENCODER_SRC_PAD(self)),
...
gst_v4l2_video_enc_register(GstPlugin *plugin, GType type,
                            const char *codec_name, const gchar *basename,
                            const gchar *device_path,
                            const GstV4l2Codec *codec, gint video_fd,
                            GstCaps *sink_caps, GstCaps *codec_caps,
                            GstCaps *src_caps)
{
