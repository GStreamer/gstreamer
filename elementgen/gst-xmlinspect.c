...
static void
output_interfaces (GType type)
{
  guint n_ifaces;
  GType *iface, *ifaces = g_type_interfaces (type, &n_ifaces);

  PUT_START_TAG (1, "interfaces");
  if (ifaces) {
    if (n_ifaces) {
      iface = ifaces;
      while (*iface) {
        PUT_STRING (2, "<interface name=\"%s\" />", g_type_name (*iface));
        iface++;
      }
    }
    g_free (ifaces);
  }
  PUT_END_TAG (1, "interfaces");
}
    ...
    switch (G_VALUE_TYPE (&value)) {
      ...
      default:
        if (param->value_type == GST_TYPE_CAPS) {
          ...
          PUT_START_TAG (pfx + 2, "flags-values");
          j = 0;
          while (values[j].value_name) {
          ...
          PUT_END_TAG (pfx + 2, "flags-values");
...
print_element_info (GstElementFactory * factory)
{
  ...
  output_interfaces (G_OBJECT_TYPE (element));
...
main (int argc, char *argv[])
{
  ...
  if (factory) {
    print_element_info (factory);
    return 0;
  }


  /* otherwise, error out */
  g_printerr ("no such plugin '%s'\n", argv[1]);
  return -1;
}
