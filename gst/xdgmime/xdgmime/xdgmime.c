...
static int
xdg_check_dir (const char *directory, int *invalid_dir_list)
{
  int invalid, exists;
  char *file_name;
...
xdg_mime_get_mime_type_for_data (const void *data, size_t len, int *result_prio)
{
  ...
  mime_type =
      _xdg_mime_magic_lookup_data (global_magic, data, len, result_prio, NULL,
      0);
    ...
    const char *mime_types[], int n_mime_types)
{
  ...
  if (_caches)
    return _xdg_mime_cache_get_mime_types_from_file_name (file_name, mime_types,
        n_mime_types);
...
xdg_mime_register_reload_callback (XdgMimeCallback callback,
    void *data, XdgMimeDestroy destroy)
{
