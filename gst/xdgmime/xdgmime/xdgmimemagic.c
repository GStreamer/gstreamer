...
_xdg_mime_magic_read_to_newline (FILE * magic_file, int *end_of_file)
{
 ...
 */
static int
_xdg_mime_magic_read_a_number (FILE * magic_file, int *end_of_file)
{
...
_xdg_mime_magic_parse_magic_line (FILE * magic_file, XdgMimeMagicMatch * match)
{
  ...
  if (c == '~') {
    matchlet->word_size =
        _xdg_mime_magic_read_a_number (magic_file, &end_of_file);
    if (end_of_file) {
      _xdg_mime_magic_matchlet_free (matchlet);
      return XDG_MIME_MAGIC_EOF;
    }
    if (matchlet->word_size != 0 &&
        matchlet->word_size != 1 &&
        matchlet->word_size != 2 && matchlet->word_size != 4) {
      _xdg_mime_magic_matchlet_free (matchlet);
      return XDG_MIME_MAGIC_ERROR;
    }
    c = getc_unlocked (magic_file);
      ...
      for (i = 0; i < matchlet->value_length; i = i + matchlet->word_size) {
        if (matchlet->word_size == 2)
          *((xdg_uint16_t *) matchlet->value + i) =
              SWAP_BE16_TO_LE16 (*((xdg_uint16_t *) (matchlet->value + i)));
        else if (matchlet->word_size == 4)
          *((xdg_uint32_t *) matchlet->value + i) =
              SWAP_BE32_TO_LE32 (*((xdg_uint32_t *) (matchlet->value + i)));
        if (matchlet->mask) {
          if (matchlet->word_size == 2)
...
_xdg_mime_magic_matchlet_compare_level (XdgMimeMagicMatchlet * matchlet,
    const void *data, size_t len, int indent)
{
...
_xdg_mime_magic_lookup_data (XdgMimeMagic * mime_magic,
    const void *data,
    size_t len, int *result_prio, const char *mime_types[], int n_mime_types)
{
  XdgMimeMagicMatch *match;
  const char *mime_type;
  int n;
...
_xdg_mime_update_mime_magic_extents (XdgMimeMagic * mime_magic)
{
  ...
  for (match = mime_magic->match_list; match; match = match->next) {
    ...
    for (matchlet = match->matchlet; matchlet; matchlet = matchlet->next) {
      ...
      extent =
          matchlet->value_length + matchlet->offset + matchlet->range_length;
...
static void
_xdg_mime_magic_read_magic_file (XdgMimeMagic * mime_magic, FILE * magic_file)
{
